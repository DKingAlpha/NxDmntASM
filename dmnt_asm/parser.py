...
class CheatParser:
    def __init__(self, err_handler=None) -> None:
        self.entries = []
        self.err_handler = err_handler if err_handler else (
            ...
            for i in range(0, len(sect_begins)):
                ...
                if line.startswith(sect_begin):
                    if not line.endswith(sect_end):
                        self.err_handler(
                            f"line #{line_number}, {line}: invalid entry declaration, missing {sect_end}")
        ...
        for entry_name, entry_block in self.entries:
            ...
            for code_no in range(0, len(entry_block)):
                ...
                try:
                    ...
                    self.err_handler(
                        f"line #{line_no}, entry {entry_name}: failed to handle `{code}`: {e}")
                    all_ok = False
